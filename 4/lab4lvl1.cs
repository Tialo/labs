...
class Level1 {
  ...
  public static void T12() {
    ...
    for (i = 0; i < 6; i++) {
      for (j = 0; j < 7; j++) {
        a[i, j] = ToDouble(ReadLine());
        if (a[i, j] > a[max_i, max_j]) { (max_i, max_j) = (i, j);
        }
      }
    }
    for (i = 0; i < 6; i++) {
  ...
  public static void T13() {
    ...
    for (i = 0; i < 5; i++) {
      for (j = 0; j < 5; j++) {
        a[i, j] = ToDouble(ReadLine());
        if (i == j && a[i, j] > a[max_i, max_j]) { (max_i, max_j) = (i, j);
        }
      }
    }
    WriteLine();
    for (i = 0; i < 5; i++) { (a[i, 4], a[i, max_j]) = (a[i, max_j], a[i, 4]);
    }
    for (i = 0; i < 5; i++) {
      for (j = 0; j < 5; j++) {
        Write($ "{a[i, j]}\t");
  ...
  public static void T17() {
    ...
    for (i = 0; i < n; i++) {
      ...
      for (j = ind; j >= 1; j--) { (a[i, j], a[i, j - 1]) = (a[i, j - 1], a[i, j]);
      }
      for (j = 0; j < m; j++) {
        Write($ "{a[i, j]}\t");
      }
